				    ...
				    void *outbuf, size_t *outlen)
{
	...
	const u32 *needle;
	u8 *tempbuf, *ptr;
	size_t templen, lzolen;
	int ret;

	lzolen = inlen + sizeof(hc_lzor_prefix);
	if (lzolen > *outlen)
		return -EFBIG;
	...
	/* LZO-decompress lzolen bytes of outbuf into the tempbuf */
	ret = lzo1x_decompress_safe(outbuf, lzolen, tempbuf, &templen);
	if (ret) {
